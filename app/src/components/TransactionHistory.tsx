      ...
      {sorted.map((tx) => (
        ...
        >
          ...
          <div className="flex-shrink-0">
            {tx.status === "success" && <CheckCircle size={16} className="text-brand-400" />}
            {tx.status === "blocked" && <XCircle size={16} className="text-red-400" />}
          ...
          <div className="flex-1 min-w-0">
            ...
            {tx.status === "blocked" && (
              <div className="text-red-400 text-xs mt-0.5 truncate">{tx.blockedReason}</div>
            )}
          ...
          <div className="text-right flex-shrink-0">
            {tx.pnl != null && (
              <div className={`flex items-center gap-1 text-xs font-mono ${tx.pnl >= 0 ? "text-brand-400" : "text-red-400"}`}>
